...
func TestHttp(t *testing.T) {
	...
	//g := gcache.NewGroup[string, []byte]("scores", 2<<10, gcache.GetterFunc[string, []byte](
	//	func(key string) ([]byte, error) {
	//		slog.Info("[SlowDB] search key", "key", key)
	//		if v, ok := db[key]; ok {
	//			return []byte(v), nil
	...
	//
	//addr := "localhost:9999"
	//peers := NewHTTPPool[string, []byte](addr, "localhost", "9999", "http")
	//slog.Info("gcache is running at", "addr", addr)
	...
	startCacheHTTPServer("localhost:8888", "localhost", "8888", "HTTP", g)
}
